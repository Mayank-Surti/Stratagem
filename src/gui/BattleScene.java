			...
			if (grid.spots[selectionX - 1][selectionY - 1]) {
				...
				case BLUE:	// blue unit attacks red unit
					if (game.redUnits[selectionX - 1][selectionY - 1] != null) {
						...
						if (game.blueUnits[currentX][currentY].getNumAttacks() >= Unit.ATTACKS_REQ) {
							game.promoteUnit(Turn.BLUE, game.blueUnits[currentX][currentY].getType(), currentX, currentY);
						}
				...
				case RED:	// red unit attacks blue unit
					if (game.blueUnits[selectionX - 1][selectionY - 1] != null) {
						...
						if (game.redUnits[currentX][currentY].getNumAttacks() >= Unit.ATTACKS_REQ) {
							game.promoteUnit(Turn.RED, game.redUnits[currentX][currentY].getType(), currentX, currentY);
						}
